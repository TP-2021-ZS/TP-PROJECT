...
def get_urls_known_source(num_urls, num_queries, kw_search_cont, kw_search_title, blacklisted_urls, date_after):
    result = []
    print("<crawling_KNOWN_urls>")
    queries = build_queries(kw_search_cont, kw_search_title, num_queries, date_after, True)
...
def get_urls_random_source(num_urls, num_queries, kw_search_cont, kw_search_title, blacklisted_urls, date_after):
    ...
    queries = build_queries(kw_search_cont, kw_search_title, num_queries, date_after, False)
    for query in queries:
        for item in crawl(query, num_urls):
            if blacklist_check(blacklisted_urls, item):
                result.append(item)
...
def build_queries(kw_search_content, kw_search_title, num_queries, date_after, url_is_known):
    queries = []
    ...
    if not url_is_known:
        ...
        for url in settings.settings.known_urls:
            """ IN TITLE QUERIES """
            for i in range(0, num_queries // 2):
                query = "site:" + url + " intitle:" + kw_search_title[random.randrange(0, end_title)].strip() + "," \
                        + kw_search_title[random.randrange(0, end_title)].strip() + " -file" + " after:" + date_after
                queries.append(query)

            """ IN TEXT QUERIES"""
            for i in range(0, num_queries // 2):
                query = "site:" + url + " intext:" + kw_search_content[random.randrange(0, end_content)].strip() + "," \
                        + kw_search_content[random.randrange(0, end_content)].strip() + \
                        "," + kw_search_content[
                            random.randrange(0, end_content)].strip() + " -file" + " after:" + date_after
                queries.append(query)
    return queries
